...
class Generate_Files extends DB_Op{
    ...
    public function calcActi($CI, $ligne, $type, $row = '', $result = ''){
        ...
        $provider_id = ($result == 'other_provider' ? $ligne->other_prov : $ligne->supplierKey);
        ...
        if ($type != 'too_exp'){
            if ($prov_delay > 0 && !is_null($prov_delay)){
                $line_delay = $this->codbu.';'.$provider_id.';'.$ligne->idProd.';'.($type == 'no_stock' ? 99 : $prov_delay);
    ...
    public function Get_Data_To_Alert($CI, $user_id){
        ...
        $query = $CI->db->get();
        log_message('error', 'Consulta: '.$CI->db->last_query());
        if ($query->num_rows > 0){
            return $query;
        }else{
            return false;
